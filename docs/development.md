...
| `tools/icon` | MAYAK のマーク（六角形と M）を `build/appicon.png`（1024px。トレイと macOS の ICNS の元）、`build/windows/icon.ico`（16〜256px）、`frontend/public/favicon-32.png` に描き、`favicon.svg` の色も揃える。`go run ./tools/icon -bg 2f3232 -fg cfd2d1`（公式ランチャーに並ぶシルバー）。 |
