...
| `tools/icon` | 原画 `tools/icon/mark.png`（六角形と M、1024px）を塗り替えて、`build/appicon.png`（トレイと macOS の ICNS の元）、`build/windows/icon.ico`（16〜256px）、`frontend/public/favicon-32.png` と `favicon-256.png`（About のロゴ）、サイトの `mayak-mark.png` と白抜きの `mayak-mark-white.png` を作る。形は原画のまま、色だけ `go run ./tools/icon -bg 1a1a1a -fg f2f2f2`（白黒）。 |
