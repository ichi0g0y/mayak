...
- **プロフィールとの紐付け**: EFT のログ（`trackerlog`）から account・profile・mode の組を検出して記憶し、組ごとにキーを 1 つ割り当てます。設定ページは上から「キー」のブロック（トークン入力、(2) 未割り当てのキー、(3) 折りたたみのキーの管理）と (1) 「EFT プロフィールとキー」の順です。追加したキーは入力のすぐ下に現れ、割り当てるとプロフィール行へ移ります。(1) 「EFT プロフィールとキー」: プロフィールごとの行（プレイ中が先頭、次に最終確認の新しい順。プレイ中は枠を強調、キーの無い行は「未割り当て」バッジ）に、付いているキー（名前とマスク済みトークン）と、同じモードのキーから選び直す小さなドロップダウン、過去ログ同期のボタン（キーがある行だけ）。そのモードのキーが 1 つも無ければその旨を出す。(2) 「未割り当てのキー」: どのプロフィールにも付いていないキーだけを点線の枠で並べ、プレイ中のプロフィールが同じモードで未割り当てなら「プレイ中のプロフィールに割り当て」の 1 クリック、ほかは未割り当てのプロフィールを選ぶドロップダウン、削除ボタン。(3) 「キーの管理（名前・削除）」: 折りたたみに全キー（付け先の表示と削除。付いているキーは削除できない）。どの割り当ても `SetTrackerProfileKey`（「未割り当て」を選ぶとそのキーをプロフィールから外す）です。
