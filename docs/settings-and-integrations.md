...
- **プロフィールとの紐付け**: EFT のログ（`trackerlog`）から account・profile・mode の組を検出して記憶し、組ごとにキーを 1 つ割り当てます。設定ページは Step 1「TarkovTracker のキーを追加する」（Step の丸いラベル付きの見出し。枠で囲まない）（トークン入力、(2) 割り当て待ちのキー、(3) 折りたたみのキーの管理）と (1) Step 2「キーを EFT プロフィールに割り当てる」の順で、両方とも同じ見た目の区切りです。追加したキーは入力のすぐ下に現れ、割り当てるとプロフィール行へ移ります。(1) Step 2「キーを EFT プロフィールに割り当てる」: モードごとのグループ（最後にプレイしたモードが先頭、その中は最終確認の新しい順。複数あるモードでは最新に「このモードの最新」バッジ）に分けたプロフィールごとの行（最後にプレイしたプロフィールは「最後にプレイ」バッジと枠の強調、キーの無い行は「未割り当て」バッジ）に、付いているキー（名前とマスク済みトークン）と、同じモードのキーから選び直す小さなドロップダウン、過去ログ同期のボタン（キーがある行だけ）。そのモードのキーが 1 つも無ければその旨を出す。(2) 「割り当て待ちのキー」: どのプロフィールにも付いていないキーだけを点線の枠で並べ、最後にプレイしたプロフィールが同じモードで未割り当てなら「最後にプレイしたプロフィールに割り当て」の 1 クリック、ほかは未割り当てのプロフィールを選ぶドロップダウン、削除ボタン。(3) 「キーの管理（名前・削除）」: 折りたたみに全キー（付け先の表示と削除。付いているキーは削除できない）。どの割り当ても `SetTrackerProfileKey`（「未割り当て」を選ぶとそのキーをプロフィールから外す）です。
