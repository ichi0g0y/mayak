export const ja = {
  ...
  start: {
    ...
    steps: [
      ...
      { title: 'スクリーンショットキーを押しやすい場所に', body: 'EFT の設定 → Controls → 「Screenshot」。既定は PrintScreen ですが、戦闘中に手が届きにくいので、マウスのサイドボタンや左手で届く空いているキー（例: 左 Ctrl + 1）への割り当てをおすすめします。座標が入るのは EFT 自身のスクリーンショット機能で保存したファイルだけで、Steam や Windows のスクリーンショットは対象外です。' },
      { title: 'レイド中にそのキーを押す。これだけで完結', body: 'MAYAK の内蔵ブラウザでマップのタブを開いておけば、設定なしで現在地が追従します。1 台の PC でマルチモニタなら、tarkov.dev との接続設定は不要です。タスク画面やアイテム詳細を撮れば、それぞれの情報も内蔵ブラウザに出ます。' },
      { title: '（任意）別の PC や普段のブラウザで見たいとき', body: 'そのブラウザで tarkov.dev のマップを開き、左下の接続ボタンを押すだけです。表示された Remote ID は、同じ PC の Chrome / Edge / Brave なら MAYAK の設定「tarkov.dev 連携」で自動検出され、別の PC やタブレットなら ID を入力します。' },
    ],
