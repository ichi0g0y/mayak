...
type Chromium struct {
	// DisableHostMessaging omits the host invocation shim for isolated browser tabs.
	DisableHostMessaging bool
	hwnd                 uintptr
	padding              struct {
		Left   int32
		Top    int32
	...
	// MAYAK: URL of the latest top-level navigation (see NavigatingURL).
	navigatingURL string
	// browserLoading is set from a page's navigation starting until it completes.
	browserLoading bool
...
func (e *Chromium) startEmbed(hwnd uintptr) error {
	...
	if e.BrowserPath != "" {
		if _, err = os.Stat(e.BrowserPath); errors.Is(err, os.ErrNotExist) {
			return fmt.Errorf("browser path '%s' does not exist", e.BrowserPath)
		}
	}
...
// EmbedAsync keeps the owning UI message loop free while WebView2 starts.
func (e *Chromium) EmbedAsync(hwnd uintptr, done func(error)) {
	e.embedCompleted = done
	if err := e.startEmbed(hwnd); err != nil {
		e.finishEmbed(err)
	}
}
func (e *Chromium) finishEmbed(err error) {
	if done := e.embedCompleted; done != nil {
		e.embedCompleted = nil
		done(err)
	}
}
...
// AddDocumentScript is Init without the process exit on failure.
func (e *Chromium) AddDocumentScript(script string) error {
	if e.webview == nil {
		return errors.New("webview is not initialised")
	}
	return e.webview.AddScriptToExecuteOnDocumentCreated(script, nil)
}
...
func (e *Chromium) CreateCoreWebView2ControllerCompleted(res uintptr, controller *ICoreWebView2Controller) uintptr {
	if int32(res) < 0 || controller == nil {
		if e.embedCompleted != nil {
			e.finishEmbed(fmt.Errorf("controller creation failed: %08x", res))
			return res
		}
	...
	if controller3 := e.controller.GetICoreWebView2Controller3(); controller3 != nil {
		...
		// Keep WebView2's automatic monitor scaling. Raw-pixel bounds do not
		// require a fixed rasterization scale; overriding it can desynchronise
		// rendering and hit-testing when the host resizes or changes monitors.
	}
