...
function aboutUpdate(){
 ...
 return `<section class="panel about-update"><h2>${esc(t('updateSection'))}</h2><dl class="about-facts"><div><dt>${esc(t('updateCurrent'))}</dt><dd>${esc(u.current||appVersion||t('aboutDev'))}</dd></div><div><dt>${esc(t('updateLatest'))}</dt><dd>${esc(u.latest||'—')}</dd></div><div><dt>${esc(t('updateCheckedAt'))}</dt><dd>${esc(checked)}</dd></div></dl><p class="update-state ${u.state==='error'?'peer-error':''}">${esc(text)}</p><div class="about-links"><button data-action="updateCheck" ${busy?'disabled':''}>${icon('reload')}${esc(t('updateCheck'))}</button>${u.state==='available'?`<button class="primary" data-action="updateDownload">${esc(t('updateDownload'))}</button>`:''}${u.state==='ready'?`<button class="primary" data-action="updateInstall">${esc(t('updateRestart'))}</button>`:''}</div>${state.localHost?`<div class="fields update-channel">${select('channel',t('updateChannel'),[['stable',t('updateChannelStable')],['nightly',t('updateChannelNightly')]],state.updateChannel||'stable','updateChannel')}</div><p class="hint">${esc(t('updateChannelHelp'))}</p>`:''}<p class="hint">${esc(t('updateAutoNote'))}</p></section>`;
}
