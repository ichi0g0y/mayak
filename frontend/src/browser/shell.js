...
function render(){
  ...
  document.documentElement.lang=state.language;document.body.dataset.layout=state.layout;document.body.dataset.item=state.itemOpen?state.itemDock:'closed';document.body.dataset.nav=state.layout==='horizontal'?'top':state.sidebarSide;document.body.dataset.settings=settingsHost()?'host':'';document.body.dataset.sidebar=state.layout==='vertical'&&state.sidebarCollapsed?'collapsed':'open';document.body.dataset.maximised=String(maximised);if(!sidebarDrag)document.documentElement.style.setProperty('--sidebar-width',state.sidebarWidth+'px');if(!itemDrag){document.documentElement.style.setProperty('--item-width',state.itemPanelWidth+'px');document.documentElement.style.setProperty('--item-height',state.itemPanelHeight+'px');}
