...
function browserSettings(key){
 switch(key){
 case 'about':return `<section class="panel about"><div class="about-head"><img src="/favicon.svg" alt="" width="56" height="56"><div><h2>MAYAK</h2><p class="about-version">${esc(t('aboutVersion'))} ${esc(appVersion||t('aboutDev'))}</p></div></div><p>${esc(t('aboutTagline'))}</p><div class="about-links"><button data-action="open" data-id="https://mayak.ich.sh">${icon('globe')}${esc(t('aboutSite'))}</button><button data-action="open" data-id="https://github.com/ichi0g0y/mayak">${icon('external')}${esc(t('aboutSource'))}</button><button data-action="open" data-id="${CHANGELOG_URL}">${icon('list')}${esc(t('aboutReleases'))}</button></div><p class="hint">${esc(t('aboutLicense'))} ${esc(t('aboutCredits'))}</p></section>`;
...
document.addEventListener('click',async event=>{
  ...
  if(type==='updateNotes'){void action('open',CHANGELOG_URL+(state.update?.latest?'#v'+state.update.latest:''));return;}
