...
Object.assign(words.ja,{bossMapBosses:'マップのボス',goonReport:'Goons を報告',goonReportOnly:'実際に Goons に遭遇したレイドだけ報告してください。報告は tarkov.dev に表示される目撃情報に加わります。',goonAccount:'報告するアカウント（EFT のログから）',goonNoAccount:'EFT のログにアカウントが見つかりません。一度ゲームを起動してください。',goonWhen:'いつ見たか',goonRaidNow:'今のレイド',goonRaidLast:'直前のレイド',goonStarted:'開始',goonAlready:'報告済み',goonWhenNow:'今の時刻で報告',goonConsent:'tarkov.dev は、報告と一緒にあなたの IP アドレスと選んだ EFT アカウント ID を保存します（不正な報告を追跡するため）。',goonSend:'報告する',goonSending:'送信中…',goonReported:'報告しました。tarkov.dev の目撃情報に反映されるまで最大10分ほどかかります。',goonCurrent:'プレイ中',goonLastSeen:'最終',bossAuto:'自動',bossModeLabel:'ゲームモード',bossPickHint:'マップを選ぶか、レイドに入るとボスを表示します',clockFormat:'時刻の表示',clock24:'24時間表示（13:05）',clock12:'12時間表示（1:05 PM）',shotKind_tasks:'タスク',shotKind_item:'アイテム',shotKind_offer:'出品',shotKind_position:'位置',shotKind_unknown:'判定なし',shotInfo:'判定の詳細',shotInfoKind:'種類',shotInfoMatch:'一致',shotInfoConfidence:'一致度',shotInfoCandidates:'候補',shotInfoLayout:'画面の形',shotInfoScore:'検出スコア',shotInfoMap:'マップ',shotInfoRaid:'レイド中',shotInfoPosition:'座標',shotInfoStage:'状態',shotInfoError:'エラー',yes:'はい',no:'いいえ',bosses:'ボス',allBosses:'ボスの詳細',bossesLoading:'ボス情報を読み込み中…',noGoons:'最近の報告なし',noBosses:'ボスは出ません',escorts:'取り巻き',goonHint:'Goons の位置はプレイヤーの報告（tarkov.dev）です。報告が新しいほど、今もそのマップにいる可能性が高くなります。',goonMaps:'Goons が出るマップ',bossMaps:'マップ',zoomIn:'拡大',zoomOut:'縮小',zoomFit:'全体を表示（リセット）',screenshots:'スクリーンショット',allScreenshots:'すべてのスクリーンショット',noScreenshots:'スクリーンショットはまだありません',screenshotsHostOnly:'スクリーンショットはHostのPCで見られます',openScreenshotFolder:'フォルダを開く',newerScreenshot:'新しい方へ',olderScreenshot:'古い方へ',
  webrtc:'インターネット経由（WebRTC・直接接続）',p2pTitle:'別のPCへ直接送信',p2pReceive:'招待コードで接続',p2pHelp:'両方のPCでMAYAKを開き、招待コードと応答コードを交換します。接続後のタスク・マップ表示指示は、暗号化して直接送ります。',p2pLimit:'STUNは接続先の探索だけに使用します。中継サーバーは使用しないため、回線によっては接続できません。アプリの再起動や接続切れの際はコードを再交換してください。',createInvite:'招待コードを作成',inviteCode:'招待コード',answerCode:'応答コード',copyCode:'コードをコピー',copied:'コピーしました',enterOffer:'Host側の招待コードを貼り付け',createAnswer:'応答コードを作成',enterAnswer:'受信側の応答コードを貼り付け',finishPairing:'接続する',disconnectPeer:'切断 / キャンセル',offerStep:'① このコードを受信側へ渡してください（有効期限10分）。',answerStep:'② このコードをHost側へ返してください。',stunLabel:'STUNサーバー',stunHelp:'空欄にすると探索サーバーも使いません。直接到達できるネットワーク向けです。',p2pIdle:'未接続',p2pGathering:'接続情報を準備しています…',p2pWaitingAnswer:'受信側の応答待ち',p2pWaitingHost:'Host側で応答コードを入力してください',p2pConnecting:'直接接続を試みています…',p2pConnected:'直接接続中（中継なし）',p2pFailed:'直接接続できませんでした。コードを作り直すか、別の回線をお試しください。',p2pDisconnected:'接続が切れました。コードを再交換してください。',p2pExpired:'招待コードの有効期限が切れました。',
});
...
function applyTheme(){
 const theme=state.theme==='system'?(lightScheme.matches?'claude-light':'claude-dark'):state.theme;
...
const settingsGroups=[['grpGeneral',['appearance','startup','sounds']],['grpGame',['folders','recognition','tasks']],['grpLinks',['remote','tracker','connection']],['grpBrowser',['adblock']],['grpDiagnostics',['status','logs','debug']]];
...
function settings(){
 ...
 return settingsHost()?'':`<div class="page settings"><h1>${esc(t('sec_'+key))}</h1>${browserSettings(key)}</div>`;
}
...
function peerPanel(){
  ...
  const statusKey={idle:'p2pIdle',gathering:'p2pGathering','waiting-answer':'p2pWaitingAnswer','waiting-host':'p2pWaitingHost',connecting:'p2pConnecting',connected:'p2pConnected',failed:'p2pFailed',disconnected:'p2pDisconnected',closed:'p2pDisconnected'}[p.phase]||'p2pIdle';
  ...
  ${!receive&&p.phase!=='connected'?`<button data-action="peerInvite" ${disabled}>${t('createInvite')}</button>`:''}
  ${receive&&(!p.code||['failed','disconnected'].includes(p.phase))&&p.phase!=='connected'?`<form id="peer-offer-form"><label class="field"><span>${t('enterOffer')}</span><textarea name="peerOffer" required spellcheck="false" maxlength="100000" rows="3">${esc(peerDrafts.offer)}</textarea></label><button type="submit" ${disabled}>${t('createAnswer')}</button></form>`:''}
  ...
  ${p.role==='sender'&&p.phase==='waiting-answer'?`<form id="peer-answer-form"><label class="field"><span>${t('enterAnswer')}</span><textarea name="peerAnswer" required spellcheck="false" maxlength="100000" rows="3">${esc(peerDrafts.answer)}</textarea></label><button class="primary" type="submit" ${disabled}>${t('finishPairing')}</button></form>`:''}
...
document.addEventListener('click',async event=>{
  ...
  if(type==='peerCopy')copied=true;
  if(['peerInvite','peerAccept','peerClose'].includes(type))copied=false;
