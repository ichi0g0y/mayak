...
import { Activity, Bug, Check, CircleX, Clock3, CloudSync, ExternalLink, FolderOpen, History, KeyRound, MapPinned, MonitorCog, Play, Plus, Radio, RefreshCw, RotateCcw, ScanLine, Search, Trash2, Volume2, Wifi, X } from 'lucide-react'
import { BrowserOpenURL, EventsOn, desktop } from './desktop'
import { GameLanguages, AnalyzeLatestScreenshot, AutoDetectEFTDirectories, AutoDetectRemoteID, ChooseLogsDirectory, ChooseScreenshotDirectory, ChooseSoundFile, ClearLogs, DiscoverTrackerProfiles, GetLogs, GetSettings, GetStatus, GetTrackerHistoryBreakpoints, ImportTrackerToken, OpenDebugDirectory, OpenLogsDirectory, OpenScreenshotDirectory, PersistSettings, PreviewSound, RefreshCatalog, OpenQuestPage, OpenHideoutDiagnostics, RefreshTracker, RemoveTrackerKey, RefreshTrackerKeyNames, SaveSettings, SetTrackerProfileKey, SyncTrackerHistory, TestRemote } from './desktop'
  ...
  useEffect(()=>{
    ...
    const boot=async()=>{
      ...
      try{
        ...
        const [s,v,l]=await Promise.all([GetSettings(),GetStatus(),GetLogs()])
        if(!active)return
