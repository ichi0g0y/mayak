...
function App(){
  ...
  const importTrackerKey=()=>run(async()=>{await ImportTrackerToken(trackerToken);setTrackerToken('')},t('trackerTokenSaved'))
  ...
  return <main className="shell">
    <Tabs className="app-tabs" value={activeTab}>
      ...
      <TabsContent value="tracker"><div className="settings-stack">
          <Card className="tracker-card"><CardHeader><div className="card-title-actions"><div className="icon-title"><CloudSync/><div><CardTitle>{t('trackerTitle')}</CardTitle><CardDescription>{t('trackerDescription')}</CardDescription></div></div><Button type="button" variant="secondary" onClick={scanTrackerProfiles} disabled={busy||!settings.logsDirectory}><RefreshCw/>{t('trackerScanLogs')}</Button></div></CardHeader><CardContent>
            ...
            {settings.tarkovTrackerEnabled&&<div className="tracker-modes">
              ...
              {status.tracker.keys.length>0&&<section className="tracker-keys" aria-labelledby="tracker-saved-keys"><Label id="tracker-saved-keys">{t('trackerSavedKeys')}</Label><small>{t('trackerKeyNamesHelp')}</small><div className="tracker-key-grid">{status.tracker.keys.map(key=><article className="tracker-key" key={key.id}><div className="tracker-key-heading"><KeyRound/><strong>{key.name||trackerModeLabel(key.mode)}</strong><Button type="button" variant="ghost" disabled={busy||key.bound} aria-label={`${t('trackerRemoveToken')}: ${key.name||key.maskedToken}`} title={key.bound?t('trackerUnassignBeforeRemove'):t('trackerRemoveToken')} onClick={()=>void removeTrackerKey(key.id)}><X/></Button></div><code>{key.maskedToken}</code><div className="tracker-key-details"><Badge>{trackerModeLabel(key.mode)}</Badge><small>{key.bound?t('trackerAssigned'):t('trackerUnassigned')}</small></div>{key.bound&&<small className="tracker-key-binding">{t('trackerAccount')} {key.accountId} · {t('trackerProfile')} {key.profileId.slice(0,6)}…{key.profileId.slice(-4)}</small>}</article>)}</div></section>}
              <div><Label>{t('trackerKnownProfiles')}</Label><p className="help">{t('trackerKnownProfilesHelp')}</p></div>
              <div className="tracker-profiles">{status.tracker.profiles.length===0?<p className="empty">{t('trackerNoProfiles')}</p>:status.tracker.profiles.map(profile=>{const keys=status.tracker.keys.filter(key=>key.mode===profile.mode&&(!key.bound||key.id===profile.boundKeyId));return <div className="tracker-profile" key={`${profile.accountId}/${profile.profileId}/${profile.mode}`}><div className="tracker-profile-info"><span><Badge>{trackerModeLabel(profile.mode)}</Badge>{profile.current&&<Badge className="current">{t('trackerCurrent')}</Badge>}</span><strong>{t('trackerAccount')} {profile.accountId}</strong><small>{t('trackerProfile')} {profile.profileId.slice(0,6)}…{profile.profileId.slice(-4)} · {new Date(profile.lastSeen).toLocaleDateString(settings.language==='ja'?'ja-JP':'en-US')}</small></div><div className="tracker-profile-actions"><Select value={profile.boundKeyId||'__none'} onValueChange={value=>void assignTrackerKey(profile,value)}><SelectTrigger aria-label={t('trackerAssignedKey')}><SelectValue/></SelectTrigger><SelectContent><SelectItem value="__none">{t('trackerUnassigned')}</SelectItem>{keys.map(key=><SelectItem key={key.id} value={key.id}>{key.name?`${key.name} · ${key.maskedToken}`:key.maskedToken}</SelectItem>)}</SelectContent></Select><Button type="button" variant="ghost" disabled={busy||!profile.boundKeyId} onClick={()=>void openTrackerHistory(profile)}><History/>{t('trackerReadPastLogs')}</Button></div></div>})}</div>
      ...
      <TabsContent value="status"><div className="status-grid">
        ...
        <Card className="wide"><CardHeader><div className="card-title-actions"><div><CardTitle>{t('updateTitle')}</CardTitle><CardDescription>{settings.autoUpdate?t('updateAutoHelp'):t('updateManualHelp')}</CardDescription></div><Button type="button" variant="secondary" disabled={busy||updateStatus.state==='checking'||updateStatus.state==='downloading'} onClick={()=>run(()=>CheckForUpdates(),t('updateChecked'))}><RefreshCw/>{t('updateCheck')}</Button></div></CardHeader><CardContent><dl><dt>{t('updateCurrent')}</dt><dd>{updateStatus.current||'—'}</dd><dt>{t('updateLatest')}</dt><dd>{updateStatus.latest||'—'}</dd><dt>{t('processState')}</dt><dd>{updateStateLabel(updateStatus)}</dd><dt>{t('updateCheckedAt')}</dt><dd>{updateStatus.checkedAt?new Date(updateStatus.checkedAt).toLocaleString(settings.language==='ja'?'ja-JP':'en-US',{hour12:hour12()}):'—'}</dd></dl>{updateStatus.state==='downloading'&&<progress className="update-progress" max={100} value={updateStatus.progress}/>}<div className="update-actions">{updateStatus.state==='available'&&<Button type="button" disabled={busy} onClick={()=>run(()=>DownloadUpdate(),t('updateDownloadStarted'))}><Download/>{t('updateDownload')}</Button>}{updateStatus.state==='ready'&&<Button type="button" disabled={busy} onClick={()=>run(()=>InstallUpdate(),t('updateRestarting'))}><RotateCcw/>{t('updateRestart')}</Button>}{updateStatus.releaseUrl&&<Button type="button" variant="secondary" onClick={()=>BrowserOpenURL(updateStatus.releaseUrl)}><ExternalLink/>{t('updateReleaseNotes')}</Button>}</div>{updateStatus.lastError&&<p className="error">{updateStatus.lastError}</p>}</CardContent></Card>
      </div></TabsContent>
